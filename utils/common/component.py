# to avoid circular loops for testing I have imported as is800 and changed to is800.IS800_2007 wherever used in code, to revert back add IS800_2007 in place of it

import utils.common.is800_2007 as is800
...
class Bolt:
    ...
    def calculate_bolt_capacity(self, bolt_diameter_provided, bolt_grade_provided, conn_plates_t_fu_fy, n_planes,
        ...
        for i in conn_plates_t_fu_fy:
            ...
            if t_fu <= t_fu_prev:
                thk_considered = i[0]
                fu_considered = i[1]
        self.d_0 = is800.IS800_2007.cl_10_2_1_bolt_hole_size(self.bolt_diameter_provided, self.bolt_hole_type)
        if self.bolt_type == "Bearing Bolt":
            self.bolt_shear_capacity = is800.IS800_2007.cl_10_3_3_bolt_shear_capacity(
                f_ub=self.bolt_fu, A_nb=self.bolt_net_area, A_sb=self.bolt_shank_area, n_n=n_planes, n_s=0)
            if seatedangle_e > 0.0:
                self.bolt_bearing_capacity = is800.IS800_2007.cl_10_3_4_bolt_bearing_capacity(
            ...
            self.beta_lg = is800.IS800_2007.cl_10_3_3_2_bolt_large_grip(d=self.bolt_diameter_provided, l_g=t_sum)

        elif self.bolt_type == "Friction Grip Bolt":
            self.bolt_shear_capacity, self.kh, self.gamma_mf = is800.IS800_2007.cl_10_4_3_bolt_slip_resistance(
    ...
    def calculate_bolt_tension_capacity(self, bolt_diameter_provided, bolt_grade_provided):
        ...
        if self.bolt_type == "Bearing Bolt":
            self.bolt_tension_capacity = is800.IS800_2007.cl_10_3_5_bearing_bolt_tension_resistance(
                f_ub=self.bolt_fu, f_yb=self.bolt_fy, A_sb=self.bolt_shank_area, A_n=self.bolt_net_area)

        elif self.bolt_type == "Friction Grip Bolt":
            self.bolt_tension_capacity = is800.IS800_2007.cl_10_4_5_friction_bolt_tension_resistance(
                f_ub=self.bolt_fu, f_yb=self.bolt_fy, A_sb=self.bolt_shank_area, A_n=self.bolt_net_area)
    ...
    def calculate_bolt_spacing_limits(self, bolt_diameter_provided, conn_plates_t_fu_fy, n=1):
        ...
        self.min_pitch = round(is800.IS800_2007.cl_10_2_2_min_spacing(self.bolt_diameter_provided), 2)
        self.min_gauge = round(is800.IS800_2007.cl_10_2_2_min_spacing(self.bolt_diameter_provided), 2)
        self.min_edge_dist = round(is800.IS800_2007.cl_10_2_4_2_min_edge_end_dist(self.bolt_diameter_provided, self.bolt_hole_type, self.edge_type), 2)
        self.min_end_dist = self.min_edge_dist
        self.max_spacing = round(is800.IS800_2007.cl_10_2_3_1_max_spacing(self.connecting_plates_tk), 2)
        self.max_edge_dist = round(is800.IS800_2007.cl_10_2_4_3_max_edge_dist(self.single_conn_plates_t_fu_fy,
        ...
        self.max_edge_dist_round = round_down(self.max_edge_dist, 5)
        self.max_end_dist_round = round_down(self.max_end_dist, 5)
        self.dia_hole = is800.IS800_2007.cl_10_2_1_bolt_hole_size(self.bolt_diameter_provided, self.bolt_hole_type)
    ...
    def calculate_combined_shear_tension_capacity(self, shear_demand, shear_capacity, tension_demand, tension_capacity, bolt_type='Bearing Bolt'):
        """ """
        if bolt_type == "Bearing Bolt":
            self.bolt_combined_capacity = is800.IS800_2007.cl_10_3_6_bearing_bolt_combined_shear_and_tension(shear_demand, shear_capacity, tension_demand,
                                                                                                       tension_capacity)  # kN
        else:  # "Friction Grip Bolt"
            self.bolt_combined_capacity = is800.IS800_2007.cl_10_4_6_friction_bolt_combined_shear_and_tension(shear_demand, shear_capacity, tension_demand,
                                                                                                       tension_capacity)  # kN
...
class Weld:
    ...
    def set_size(self, weld_size, fusion_face_angle=90):
        ...
        self.size = weld_size
        self.throat_tk = is800.IS800_2007.cl_10_5_3_2_fillet_weld_effective_throat_thickness(
            fillet_size=self.size, fusion_face_angle=fusion_face_angle)
        self.eff_length = is800.IS800_2007.cl_10_5_4_1_fillet_weld_effective_length(
            fillet_size=self.size, available_length=self.length)
        self.lj_factor = is800.IS800_2007.cl_10_5_7_3_weld_long_joint(l_j=self.eff_length, t_t=self.throat_tk)

    def set_min_max_sizes(self, part1_thickness, part2_thickness, special_circumstance=False, fusion_face_angle=90):
        self.min_size = is800.IS800_2007.cl_10_5_2_3_min_weld_size(part1_thickness, part2_thickness)
        k = is800.IS800_2007.cl_10_5_3_2_factor_for_throat_thickness(fusion_face_angle)
        self.max_size = (is800.IS800_2007.cl_10_5_3_1_max_weld_throat_thickness(part1_thickness, part2_thickness, special_circumstance)) / k

    def get_weld_strength(self, connecting_fu, weld_fabrication, t_weld, weld_angle):
        # connecting_fu.append(self.fu)
        f_wd = is800.IS800_2007.cl_10_5_7_1_1_fillet_weld_design_stress(connecting_fu, weld_fabrication)
        self.throat_tk = \
            round(is800.IS800_2007.cl_10_5_3_2_fillet_weld_effective_throat_thickness \
                      (t_weld, weld_angle), 2)
    ...
    def get_weld_red(self, t_t, strength, height=0.0, length=0.0):
        ...
        beta_lw = is800.IS800_2007.cl_10_5_7_3_weld_long_joint(lj, t_t)
...
class Plate(Material):
    ...
    def tension_blockshear_area_input(self, A_vg, A_vn, A_tg, A_tn, f_u, f_y):
        ...
        """
        gamma_m0 = is800.IS800_2007.cl_5_4_1_Table_5["gamma_m0"]['yielding']
        gamma_m1 = is800.IS800_2007.cl_5_4_1_Table_5["gamma_m1"]['ultimate_stress']
    ...
    def cleat_angle_check(self, h, t, nr, nc, p, en, g, ed, dh, fu, fy, m_d, h_max, v, n=2):
        self.design_status = False
        self.cleat_shear_capacity = is800.IS800_2007.cl_8_4_design_shear_strength(n*h*t, fy)
        self.cleat_moment_capacity = is800.IS800_2007.cl_8_2_1_2_design_moment_strength(n*t*h*h/6, n*t*h*h/4, fy, 'plastic')
